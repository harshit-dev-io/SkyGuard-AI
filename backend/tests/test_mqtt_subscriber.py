...
@pytest.mark.asyncio
async def test_subscriber_default_topic_and_init():
...
@pytest.mark.asyncio
async def test_handle_message_valid_payload():
    orchestrator = MagicMock(spec=CentralPipelineOrchestrator)
    subscriber = AsyncMQTTSubscriber(orchestrator=orchestrator)
...
@pytest.mark.asyncio
async def test_subscriber_subscription_and_loop():
...
@pytest.mark.asyncio
async def test_reconnection_resilience():
    mock_orchestrator = MagicMock(spec=CentralPipelineOrchestrator)
    subscriber = AsyncMQTTSubscriber(
        broker_host="localhost",
        broker_port=1883,
